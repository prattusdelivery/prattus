...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (acao === 'excluir_restaurante') {
      // Confirma que o restaurante existe e pega o user_id do dono
      const restResp = await fetch(`${SUPABASE_URL}/rest/v1/restaurantes?id=eq.${restauranteId}&select=id,user_id,nome`, { headers: svcHeaders });
      const restData = await restResp.json();
      const restaurante = Array.isArray(restData) ? restData[0] : null;
      if (!restaurante) return res.status(404).json({ error: 'Restaurante não encontrado' });

      const del = async (tabela, filtro) => {
        try {
          await fetch(`${SUPABASE_URL}/rest/v1/${tabela}?${filtro}`, { method: 'DELETE', headers: { ...svcHeaders, 'Prefer': 'return=minimal' } });
        } catch (e) {}
      };

      // 1. pedido_itens (dependem dos pedidos, não têm restaurante_id direto)
      try {
        const pedResp = await fetch(`${SUPABASE_URL}/rest/v1/pedidos?restaurante_id=eq.${restauranteId}&select=id`, { headers: svcHeaders });
        const pedData = await pedResp.json();
        const pedIds = (Array.isArray(pedData) ? pedData : []).map(p => p.id);
        if (pedIds.length) {
          await del('pedido_itens', `pedido_id=in.(${pedIds.join(',')})`);
        }
      } catch (e) {}

      // 2. tabelas com restaurante_id direto
      await del('pedidos', `restaurante_id=eq.${restauranteId}`);
      await del('codigos_comanda', `restaurante_id=eq.${restauranteId}`);
      await del('cupons', `restaurante_id=eq.${restauranteId}`);
      await del('etiquetas', `restaurante_id=eq.${restauranteId}`);
      await del('eventos_plano', `restaurante_id=eq.${restauranteId}`);
      await del('fretes', `restaurante_id=eq.${restauranteId}`);
      await del('itens', `restaurante_id=eq.${restauranteId}`);
      await del('categorias', `restaurante_id=eq.${restauranteId}`);
      await del('recompensas', `restaurante_id=eq.${restauranteId}`);
      await del('push_subscriptions', `restaurante_id=eq.${restauranteId}`);
      await del('restaurante_privado', `restaurante_id=eq.${restauranteId}`);

      // 3. clientes — apaga o login de cada um (se tiver e não for usado em outro restaurante)
      try {
        const cliResp = await fetch(`${SUPABASE_URL}/rest/v1/clientes?restaurante_id=eq.${restauranteId}&select=id,user_id`, { headers: svcHeaders });
        const cliData = await cliResp.json();
        for (const c of (Array.isArray(cliData) ? cliData : [])) {
          if (c.user_id) {
            try {
              const outroResp = await fetch(`${SUPABASE_URL}/rest/v1/clientes?user_id=eq.${c.user_id}&restaurante_id=neq.${restauranteId}&select=id&limit=1`, { headers: svcHeaders });
              const outroData = await outroResp.json();
              if (!Array.isArray(outroData) || outroData.length === 0) {
                await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${c.user_id}`, { method: 'DELETE', headers: svcHeaders });
              }
            } catch (e) {}
          }
        }
      } catch (e) {}
      await del('clientes', `restaurante_id=eq.${restauranteId}`);

      // 4. equipe — apaga os logins sintéticos de cada membro
      try {
        const eqResp = await fetch(`${SUPABASE_URL}/rest/v1/usuarios_restaurante?restaurante_id=eq.${restauranteId}&select=id,user_id`, { headers: svcHeaders });
        const eqData = await eqResp.json();
        for (const u of (Array.isArray(eqData) ? eqData : [])) {
          if (u.user_id) {
            try { await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${u.user_id}`, { method: 'DELETE', headers: svcHeaders }); } catch (e) {}
          }
        }
      } catch (e) {}
      await del('usuarios_restaurante', `restaurante_id=eq.${restauranteId}`);

      // 5. o restaurante em si
      await del('restaurantes', `id=eq.${restauranteId}`);

      // 6. por último, o login do dono (se algo acima falhar, o dono ainda consegue entrar pra você tentar de novo)
      if (restaurante.user_id) {
        try { await fetch(`${SUPABASE_URL}/auth/v1/admin/users/${restaurante.user_id}`, { method: 'DELETE', headers: svcHeaders }); } catch (e) {}
      }
